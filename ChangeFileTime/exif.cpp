 ...
 */
//
// Modified by Damir Valiulin on 2010-05-08.
//    * Brought minor changes from CXImage
//    * Fixed a bug with 0th IFD offset
//
// Used as part of
...
////////////////////////////////////////////////////////////////////////////////
bool Cexif::DecodeExif(FILE * hFile, char* pDateTaken, bool bUpdate)
{
    int a;
    int HaveCom = 0;
    ...
    for(;;){
        ...
        if (SectionsRead >= MAX_SECTIONS){
			strcpy_s(m_szLastError, "Too many sections in jpg file");
			return 0;
        }
        ...
        for (a=0;a<7;a++){
            ...
            if (a >= 6){
					strcpy_s(m_szLastError,"too many padding bytes!");
                return 0;
            }
        }
        ...
        if (itemlen < 2){
            strcpy_s(m_szLastError,"invalid marker");
			return 0;
        }
        ...
        if (Data == NULL){
            strcpy_s(m_szLastError,"Could not allocate memory");
			return 0;
        }
        ...
        if (got != itemlen-2){
            strcpy_s(m_szLastError,"Premature end of file?");
			return 0;
        }
        SectionsRead += 1;
        ...
        switch(marker){
            ...
            case M_EOI:   // in case it's a tables-only JPEG stream
                strcpy_s(m_szLastError,"No image in jpeg!");
                return 0;
            ...
            case M_EXIF:
                if (memcmp(Data+2, "Exif", 4) == 0){
                    m_exifinfo->IsExif = process_EXIF((unsigned char *)Data+2, itemlen, pDateTaken, bUpdate);
					
					fseek(hFile, 2 - itemlen, SEEK_CUR);
					fwrite(Data+2, 1, itemlen-2, hFile);
					fseek(hFile, 2 - itemlen, SEEK_CUR);
                }else{
                    // Discard this section.
                    free(Sections[--SectionsRead].Data);
...
--------------------------------------------------------------------------*/
bool Cexif::process_EXIF(unsigned char * CharBuf, unsigned int length, char* pDateTaken, bool bUpdate)
{
    ...
    {   /* Check the EXIF header component */
        ...
        if (memcmp(CharBuf+0, ExifHeader,6)){
			strcpy_s(m_szLastError,"Incorrect Exif header");
			return 0;
		}
    }
    ...
    if (memcmp(CharBuf+6,"II",2) == 0){
        ...
        if (memcmp(CharBuf+6,"MM",2) == 0){
            ...
            strcpy_s(m_szLastError,"Invalid Exif alignment marker.");
			return 0;
        }
    }
    ...
    if (Get16u(CharBuf+8) != 0x2a){
        strcpy_s(m_szLastError,"Invalid Exif start (1)");
		return 0;
    }
    ...
    unsigned char * LastExifRefd = CharBuf;
	 ...
	 /* Read directory at offset */
	 if (!ProcessExifDir(CharBuf+14+FirstOffset-8, CharBuf+6, length-6, m_exifinfo, &LastExifRefd, 0, pDateTaken, bUpdate) )
		 return 0;
...
bool Cexif::ProcessExifDir(unsigned char * DirStart, unsigned char * OffsetBase, unsigned ExifLength,
                           EXIFINFO * const m_exifinfo, unsigned char ** const LastExifRefdP, int NestingLevel  ,char* pDateTaken, bool bUpdate)
{
    int de;
    int a;
    int NumDirEntries;
    ...
    unsigned ThumbnailSize = 0;
	 ...
	 if (NestingLevel > 4){
		 strcpy_s(m_szLastError,"Maximum directory nesting exceeded (corrupt exif header)");
		 return false;
	 }
    ...
    if ((DirStart+2+NumDirEntries*12) > (OffsetBase+ExifLength)){
        strcpy_s(m_szLastError,"Illegally sized directory");
		return 0;
    }
    ...
    for (de=0;de<NumDirEntries;de++){
        ...
        if ((Format-1) >= NUM_FORMATS) {
            ...
            strcpy_s(m_szLastError,"Illegal format code in EXIF dir");
			return 0;
		}
        ...
        if (BytesCount > 4){
            ...
            if (OffsetVal+BytesCount > ExifLength){
                ...
                strcpy_s(m_szLastError,"Illegal pointer offset value in EXIF.");
				return 0;
            }
        ...
        switch(Tag){
            ...
            case TAG_MAKE:
                strncpy_s(m_exifinfo->CameraMake, (char*)ValuePtr, 31);
                break;

            case TAG_MODEL:
                strncpy_s(m_exifinfo->CameraModel, (char*)ValuePtr, 39);
                break;

			case TAG_EXIF_VERSION:
				strncpy_s(m_exifinfo->Version,(char*)ValuePtr, 4);
				break;

            case TAG_DATETIME_ORIGINAL:
				{
				strncpy_s(m_exifinfo->DateTime, (char*)ValuePtr, 39);

				if ( bUpdate)
					memcpy(ValuePtr, pDateTaken, strlen(pDateTaken));
                break;
				}
            case TAG_USERCOMMENT:
                ...
                if (memcmp(ValuePtr, "ASCII",5) == 0){
                    for (a=5;a<10;a++){
                        ...
                        c = ((char*)ValuePtr)[a];
                        if (c != '\0' && c != ' '){
                            strncpy_s(m_exifinfo->Comments, (char*)ValuePtr+a, 199);
                            break;
                        }
                    }
                    
                }else{
                    strncpy_s(m_exifinfo->Comments, (char*)ValuePtr, 199);
                }
                break;
            ...
            case TAG_ORIENTATION:
                ...
                if (m_exifinfo->Orientation < 1 || m_exifinfo->Orientation > 8){
                    strcpy_s(m_szLastError,"Undefined rotation value");
				...
				if (Offset>8){
					...
					if (SubdirStart < OffsetBase || 
						SubdirStart > OffsetBase+ExifLength){
							strcpy_s(m_szLastError,"Illegal subdirectory link");
							return 0;
					}
					ProcessExifDir(SubdirStart, OffsetBase, ExifLength, m_exifinfo, LastExifRefdP, NestingLevel+1, pDateTaken, bUpdate);
				}
            continue;
        }
    }
...
void Cexif::process_COM (const unsigned char * Data, int length)
{
    ...
    strcpy_s(m_exifinfo->Comments,Comment);
}
////////////////////////////////////////////////////////////////////////////////
