...
`Tests/PhotoTimeTests` includes `ChangeFileTime/EXIF.H` and checks JPEG marker constants (`M_SOI`, `M_EOI`, `M_EXIF`), `MAX_SECTIONS`, and `EXIF_READ_ALL`. Build and run:

```
msbuild Tests\PhotoTimeTests\PhotoTimeTests.vcxproj /p:Configuration=Release /p:Platform=x64
Tests\PhotoTimeTests\x64\Release\PhotoTimeTests.exe
```
